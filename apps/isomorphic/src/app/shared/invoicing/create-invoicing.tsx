...
export const CreateInvoice = ({ getInvoices }: CreateInvoiceProps) => {
   ...
   const onSubmit = async (data: InvoiceFormData) => {
      ...
      try {
         ...
         getInvoices();
         reset();
         setSelectedCustomerRetIss(false); // Reset do estado
         closeModal();
      } catch (error: any) {
         console.error('Erro ao criar fatura:', error);
         const errorMessage = error.response?.data?.message || 'Erro ao criar fatura';
      ...
      <form className="flex w-[100%] flex-col items-center justify-center" onSubmit={handleSubmit(onSubmit)}>
         <div className="w-full space-y-5">
            ...
            <Button disabled={loading} className="w-full" type="submit" size="lg">
               <span>{loading ? 'Criando...' : 'Criar Fatura'}</span>
            </Button>
         </div>
      </form>
   );
