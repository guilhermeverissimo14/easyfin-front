...
// Legendas traduzidas
export const cashFlowLegend = [{ name: 'Contas a Receber' }, { name: 'Contas a Pagar' }];
      ...
      <WidgetCard
         ...
         action={
            <div className="flex gap-5">
               ...
               <DropdownAction onChange={handleChange} className="rounded-md border" options={financialViewOptions} dropdownClassName="!z-0" />
            </div>
         }
      >
                  ...
                  <ResponsiveContainer width="100%" height="100%" minWidth={1450}>
                     ...
                     >
                        ...
                        <YAxis
                           axisLine={false}
                           tickLine={false}
                           tickFormatter={(label) => `R$ ${Number(label).toFixed(2).toString().replace('.', ',')}`}
                           width={100}
                        />
                        <Tooltip content={<CustomTooltip />} cursor={false} />
                        <Bar dataKey="contasReceber" fill={CASH_FLOW_COLORS[0]} barSize={28} radius={[4, 4, 0, 0]} />
                        <Bar type="natural" dataKey="contasPagar" fill={CASH_FLOW_COLORS[1]} barSize={28} radius={[4, 4, 0, 0]} />
                     </ComposedChart>
                  </ResponsiveContainer>
               </div>
            </div>
...
function CustomLegend({ className }: { className?: string }) {
   return (
      <div className={cn('mt-2 flex flex-wrap items-start gap-3 lg:gap-7', className)}>
         {cashFlowLegend.map((item, index) => (
            <div key={item.name} className="flex items-center gap-1.5">
               <span className="h-3 w-3 rounded-full" style={{ backgroundColor: CASH_FLOW_COLORS[index] }} />
               <span>{item.name}</span>
            </div>
         ))}
      </div>
